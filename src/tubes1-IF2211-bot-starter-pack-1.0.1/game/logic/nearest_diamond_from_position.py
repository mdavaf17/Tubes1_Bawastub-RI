...
class NearestDMCurrPost(BaseLogic):
    def __init__(self):
    ...
    def next_move(self, board_bot: GameObject, board: Board):
        ...
        for d in diamond_board:
            ...
            # compare candidate with temp nearest
                ...
                if (
                    abs(curr_post.x - d.position.x)
                    + abs(curr_post.y - d.position.y)
                    < abs(curr_post.x - self.goal_position.x)
                    + abs(curr_post.y - self.goal_position.y)
                ):
                    self.goal_position = d.position
